    ...
    F: Fn(&mut T::Native, T::Native) + Send + Sync,
{
    ...
    /// Mark that `starting_value` is the identity element for `prim_fn`,
    /// i.e. `prim_fn(starting_value, x) == x` for all x (e.g. 0 for
    /// addition, 0 for bitwise OR/XOR). This allows `convert_to_state`
    /// to skip allocating an initial-state array and the element-wise
    /// arithmetic, returning the input values directly.
