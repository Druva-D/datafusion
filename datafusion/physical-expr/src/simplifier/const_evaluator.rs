...
/// Check if an expression can be evaluated as a constant (no columns, not volatile).
/// Single-pass replacement for the previous `is_volatile() || has_column_references()`.
